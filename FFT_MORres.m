...
disp(' ')
%% Make C matrix
disp('----COMPUTING C MATRIX---------------------------------------------')
tic
[C] = fun_make_C(L,M,N,K,idxV,Kt,idxF);
toc
disp(' ')
...
for ii = 1:Nmat
    if strcmp(Ind(ii).tag,'cond') 
         Ind(ii).rho=[Ind(ii).rhomin,Ind(ii).rhomax];
    end
end
for ii = 1:Nmat
        rhoVoxel=zeros(nVoxel,1);
        rhoVoxel(Ind(ii).ind,1)=1;
        rho_eV=reshape(rhoVoxel,L,M,N);
        rho_eF=0.5*(abs(Ae(:,:)).'*rho_eV(:));
        z_realF=rho_eF;
        indFneq=setdiff([1:3*Kt].',[idxFx;idxFy+Kt;idxFz+2*Kt]);
        z_realF(indFneq,:)=0;
        z_realx=zeros(L,M,N);
...
%%
disp('----GENERATING MOR-------------------------------------------------')
[yR,yA,yL,V,b_hat]=fun_mor_mat5(mor.n,mor.tol,mor.max,rhs_vectR,R,Aee,AeeR,opt_amg_MATMAG,...
                        opt_amg_AGMG,L,M,N,...
                        dx,dy,dz,Ind,idxF,...
...
Irho=(rand(Nmat,1)-0.5)*2; % generate random resistivity pattern, for each domain a number between -1 and +1 is selected: -1-> minimum value of the resistivity, +1->maximum value of the resistivity
rhopoint=0.5*(mat_rho(:,2)+mat_rho(:,1))+Irho.*0.5.*(mat_rho(:,2)-mat_rho(:,1));
yRloc=yR(1).R*0;
for ii = 1:Nmat
    yRloc=yRloc+rhopoint(ii)*yR(ii).R;
end
A_hat = yRloc+yA+yL; % assembling ROM for the chosen resistivity 
%
x_hat = A_hat \ (b_hat); % solving ROM
vsol=V*x_hat; % from ROM to FOM
%% extract solution
Jout = zeros(L,M,N,3);
