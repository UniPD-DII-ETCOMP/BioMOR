...
% The original function pgmres_iter.m is available in the directory 
...
function [x,r,k,resvec] = pgmres_iter_mod(A,x,r,m,L1,L2,R1,R2,tol)
...
disp(['  res:', num2str(resvec(k)/rnorm)]) % MODIFIED_BY_RICCARDO_TORCHIO
% figure(9876) % MODIFIED_BY_RICCARDO_TORCHIO
% semilogy(resvec/rnorm,'-x') % MODIFIED_BY_RICCARDO_TORCHIO
% title('gmres')  % MODIFIED_BY_RICCARDO_TORCHIO
% drawnow % MODIFIED_BY_RICCARDO_TORCHIO
